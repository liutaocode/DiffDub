...
class GaussianDiffusionBeatGans:
    ...
    def training_losses(self,
                        ...
                        x_start: th.Tensor,
                        ref_imgs: th.Tensor,
                        x_aug_cond: th.Tensor,
                        t: th.Tensor,
                        mask: th.Tensor,
                        noise: th.Tensor = None, **model_kwargs):
        """
        ...
        x_t = ( 1 - mask ) * x_t + mask * x_start
        
        if ref_imgs is not None:
            x_t_ref = torch.cat((x_t, ref_imgs), dim=1)
        else:
            x_t_ref = x_t
        ...
        if self.loss_type in [
            ...
            with autocast(self.conf.fp16):
                ...
                model_forward = model.forward(x=x_t_ref.detach(),
                                              t=self._scale_timesteps(t),
                                              x_start=x_aug_cond.detach(),
                                              **model_kwargs)
            ...
            if self.loss_type == LossType.mse:
                if self.model_mean_type == ModelMeanType.eps:
                    # (n, c, h, w) => (n, )
                    terms["mse"] = mean_flat(( 1 - mask )*((target - model_output)**2))
                else:
                    raise NotImplementedError()
            elif self.loss_type == LossType.l1:
                # (n, c, h, w) => (n, )
                terms["mse"] = mean_flat(( 1 - mask )*(target - model_output).abs())
            else:
                raise NotImplementedError()
    ...
    def p_mean_variance(self,
        ...
        assert t.shape == (B, )
        with autocast(self.conf.fp16):
            model_forward = model.forward(x=x_cat,
                                          t=self._scale_timesteps(t),
                                          **model_kwargs)
        model_output = model_forward.pred
    ...
    def ddim_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                img = ( 1 - mask ) * img + mask * x_start
                
                # print(img.shape)
                # pred_img = (img + 1) / 2
                # image = pred_img[0].detach().permute(1,2,0).cpu().numpy()*255
                # image = image.astype(np.uint8)
                # image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)[80:230,60:220-80+60]
                # cv2.imwrite('tmp_%d.png'%(i), image.astype(np.uint8))
                
                
                # image = mask[0].detach().permute(1,2,0).cpu().numpy()*255
                # image = image.astype(np.uint8)
                # image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)[80:230,60:220-80+60]
                # cv2.imwrite('mask.png', image.astype(np.uint8))
                ...
                if x_ref is None:
                    ...
                    x_t_ref = torch.cat((img, x_ref), dim=1)
